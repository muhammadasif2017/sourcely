...
- **Who is calling.** `app/api/auth.py`: `SessionDep` (cookie + CSRF), `PrincipalDep` (session + `X-Workspace-ID`, for data routes), `MemberDep` (session + `{workspace_id}` path, for management routes). Check roles with `require(principal, action)`, where action is `read`, `write`, `manage` or `own`. The role table lives only in `app/services/workspaces.py` (`allowed`).
