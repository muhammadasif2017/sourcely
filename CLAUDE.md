...
- **Gemini answers a bad key with 400 `INVALID_ARGUMENT` "Please pass a valid API key", not 401.** It maps to 502 like any rejected request. An OpenAI `sk-proj-…` key with the Gemini base URL produces exactly this. Gemini keys start with `AIza` or, for newer AI Studio keys, `AQ.`.
...
- **Failed sign-ins are recorded in their own transaction** (`accounts.record_failed_login(engine, ...)`). The request's `DbDep` transaction rolls back when the route raises, which would otherwise erase the record and disable the throttle.
- **`AppError` takes `headers=`** for things like `Retry-After`.
- **Role rules for people** (`app/services/workspaces.py`): `change_role` and `remove_member` only act on members ranked below the caller, and only grant roles below the caller's; the owner changes only by transfer; anyone but the owner can leave. Invites follow the same "below your own role" rule (`app/services/invites.py`).
...
- **Tests use cheap Argon2 parameters** (session fixture `fast_password_hashing` swaps `security._hasher`). Production uses `production_password_hasher()`, and `test_production_hasher_uses_rfc_9106_parameters` guards it. The full suite takes about 1.5 minutes, mostly per-test client start-up and table truncation.
