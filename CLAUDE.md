...
- **Test fixtures:** `anon_client` (no credentials), `app`, `outbox` (captured emails; `outbox.last(email)`), `owner_db` (autocommit connection as the owner, for backdating rows). Test settings use `cookie_secure=False`, because TestClient speaks plain HTTP.
