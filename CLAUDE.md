...
- **OS environment variables override `.env`** (pydantic-settings precedence). This machine has a user-level `OPENAI_API_KEY` holding an OpenAI key, which silently replaces the Gemini key from `.env`. Until it's removed, run live checks with `env -u OPENAI_API_KEY uv run ...`. Tests are unaffected: they build `Settings(_env_file=None, ...)` with explicit values.
